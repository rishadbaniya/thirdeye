...
fn default_no_of_devices() -> Option<i32> {
    Some(0)
}
fn default_devices() -> Option<Vec<String>> {
    Some(vec![])
}
...
pub async fn create_group(
    ...
    match groups_collection.find_one(group_filter_option, None).await {
        ...
        Ok(None) => {
            match groups_collection
                .insert_one(new_group, None)
                .await
                .unwrap()
                .inserted_id
            {
...
pub async fn get_groups(
    ...
    let group_find_options = FindOptions::builder()
    ...
    match groups_collection.find(None, Some(group_find_options)).await {
        Ok(mut cursor) => {
            ...
            let count = groups_collection
                .count_documents(None, None)
                .await
                .map_err(|err| {
                    log::error!(
                        "Database Error while trying to count total no of groups| ${err:?}"
                    );
...
pub async fn delete_group(
    ...
    let result = groups_collection
        .delete_one(group_filter_option, None)
        .await
