use actix_web::{post, get, delete, put, Responder, web, HttpResponse,  HttpRequest, Result, error, dev};
...
#[post("/device")]
pub async fn create_device(mongodb_client: web::Data<MongoDBClient>, new_device: web::Json<NewDevice>) -> Result<impl Responder>{
   ...
   match devices_collection.find_one(group_filter_option, None).await {
       ...
       Ok(None) => {
            match devices_collection.insert_one(new_device, None).await.unwrap().inserted_id {
